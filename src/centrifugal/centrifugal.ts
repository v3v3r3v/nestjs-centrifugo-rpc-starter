...
export interface CentrifugoProxyClient {
  ...
  publish(request: PublishRequest): Observable<PublishResponse>;

  /** RPC to execute custom logic on the backend over request through the real-time connection. */

  rpc(request: RPCRequest): Observable<RPCResponse>;

  /** SubRefresh to proxy decision about subscription expiration to the app backend. */

  subRefresh(request: SubRefreshRequest): Observable<SubRefreshResponse>;
  ...
  notifyChannelState(request: NotifyChannelStateRequest): Observable<NotifyChannelStateResponse>;
}

export interface CentrifugoProxyController {
  /** Connect to proxy connection authentication and communicate initial state. */

  connect(request: ConnectRequest): Promise<ConnectResponse> | Observable<ConnectResponse> | ConnectResponse;
  ...
  publish(request: PublishRequest): Promise<PublishResponse> | Observable<PublishResponse> | PublishResponse;

  /** RPC to execute custom logic on the backend over request through the real-time connection. */

  rpc(request: RPCRequest): Promise<RPCResponse> | Observable<RPCResponse> | RPCResponse;
